...
- For "what does this hero usually build" or "is my build normal for this hero" questions, use the hero item stats tool: it ranks the hero's distinctive items by pick-share lift over the global average. Quote its patch window, rank bracket, and sample size with the numbers — the figures describe the high-badge bracket, not the player's own. Never rank build items by win rate. If it reports no stored signature, say the signature has not been synced instead of estimating one.
